...
func (r *VerticalPodAutoscalerReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if errGet := r.Get(ctx, req.NamespacedName, &clientVPA); errGet != nil {
		...
		if !utils.IsTargetWorkloadSupported(schema.GroupVersionKind{Group: strings.Split(clientVPA.Spec.TargetRef.APIVersion, "/")[0], Version: strings.Split(clientVPA.Spec.TargetRef.APIVersion, "/")[1], Kind: clientVPA.Spec.TargetRef.Kind}) {
			logger.Info("The VPA targets something else than a supported workload.", "name", req.Name, "namespace", req.Namespace, "targetKind", clientVPA.Spec.TargetRef.Kind, "targetAPIVersion", clientVPA.Spec.TargetRef.APIVersion)
			return ctrl.Result{}, nil
		}
		if cachedVPA, isInCache := clientVPACache[req.NamespacedName]; isInCache {
