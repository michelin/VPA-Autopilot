...
var _ = Describe("VerticalPodAutoscaler Controller", func() {
	...
	Context("When reconciling a resource", func() {
		for _, workloadType := range utils.ListKindSupported {
			// Test that the automatic VPA is deleted if another VPA that targets the same workload is created
			It(fmt.Sprintf("Deletes the automatic VPA if another VPA targeting the %s is created", workloadType.Kind), func() {
				...
				By(fmt.Sprintf("Creating another VPA for the %s", workloadType.Kind))
				clientVPA := testutils.GenerateTestClientVPA(ctx, workload.GetNamespace(), workloadType.ApiVersion, workloadType.Kind, workload.GetName())
			...
			It(fmt.Sprintf("Deletes the automatic VPA if another VPA targeting the %s in lowercase is created", workloadType.Kind), func() {
				By("Setting up an automatic VPA")
			...
			// Test that the automatic VPA is created if the other VPA that targets the same workload is deleted
			It(fmt.Sprintf("Creates the automatic VPA if the other VPA targeting the %s is deleted", workloadType.Kind), func() {
				By(fmt.Sprintf("Creating a VPA for the future %s", workloadType.Kind))
				clientVPA := testutils.GenerateTestClientVPA(ctx, "default", workloadType.ApiVersion, workloadType.Kind, "test-delete-clientvpa-creates-automaticvpa")
				Expect(k8sClient.Create(ctx, clientVPA)).To(Succeed())
			...
			It(fmt.Sprintf("Deletes the automatic VPA if another VPA is updated to target the %s", workloadType.Kind), func() {
				By(fmt.Sprintf("Creating a test %s", workloadType.Kind))
				workload, err := testutils.GenerateTestWorkload(workloadType.Kind)
				Expect(err).ToNot(HaveOccurred())
				Expect(k8sClient.Create(ctx, workload)).To(Succeed())
			...
			It(fmt.Sprintf("Updates the automatic VPAs correctly when another VPA changes %s targets", workloadType.Kind), func() {
				...
				By(fmt.Sprintf("Creating another VPA for the %s 1", workloadType.Kind))
				clientVPA := testutils.GenerateTestClientVPA(ctx, workload1.GetNamespace(), workloadType.ApiVersion, workloadType.Kind, workload1.GetName())
				Expect(k8sClient.Create(ctx, clientVPA)).To(Succeed())
				defer func() {
					Expect(k8sClient.Delete(ctx, clientVPA)).To(Succeed())
				}()
				...
				clientVPA.Spec.TargetRef.Name = workload2.GetName()
			...
			It(fmt.Sprintf("Updates the automatic VPAs correctly when another VPA changes from and to target a non existing %s", workloadType.Kind), func() {
				By(fmt.Sprintf("Creating another VPA for a non existing %s", workloadType.Kind))
				clientVPA := testutils.GenerateTestClientVPA(ctx, "default", workloadType.ApiVersion, workloadType.Kind, "i-do-not-exist")
				Expect(k8sClient.Create(ctx, clientVPA)).To(Succeed())
				defer func() {
					Expect(k8sClient.Delete(ctx, clientVPA)).To(Succeed())
				}()
