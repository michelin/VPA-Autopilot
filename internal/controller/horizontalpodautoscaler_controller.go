...
func (r *HorizontalPodAutoscalerReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if errors.IsNotFound(errV1) && errors.IsNotFound(errV2) {
		...
		if !utils.IsTargetWorkloadSupported(schema.GroupVersionKind{Group: strings.Split(clientHPATarget.APIVersion, "/")[0], Version: strings.Split(clientHPATarget.APIVersion, "/")[1], Kind: clientHPATarget.Kind}) {
			logger.Info("The HPA targets something else than a supported workload.", "name", req.Name, "namespace", req.Namespace, "targetKind", clientHPATarget.Kind, "targetAPIVersion", clientHPATarget.APIVersion)
			return ctrl.Result{}, nil
		}
		if cachedHPA, isInCache := clientHPACache[req.NamespacedName]; isInCache {
