...
// AutoVPAStatefulSetReconciler reconciles a VerticalPodAutoscaler object
type AutoVPAStatefulSetReconciler struct {
	client.Client
...
func (r *AutoVPAStatefulSetReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	logger := log.FromContext(ctx)

	// Get workload namespace to check if the namespace is ignored
	...
	logger.Info("Begin reconcile automatic VPA for workload", "name", req.Name, "namespace", req.Namespace)

	// Get the statefulset complete object
	var statefulset appsv1.StatefulSet
	if err := r.Get(ctx, req.NamespacedName, &statefulset); err != nil {
		// here, we could not get the statefulset object
		if errors.IsNotFound(err) {
			// here, the statefulset corresponding to the event being processed does not exist
			...
			logger.Info("StatefulSet is deleted", "name", req.Name, "namespace", req.Namespace)
			return ctrl.Result{}, nil
		}
		logger.Error(err, "unable to fetch statefulset")
		// we got an error getting the StatefulSet but this was not a NotFound error
		// let's return an error to indicate that the operation failed and that the
		// controller should requeue the request
		return ctrl.Result{}, err
	}

	logger.Info("Handling VPA for statefulset", "name", req.Name, "namespace", req.Namespace)
	...
	// Calling the common reconcile function with the statefulset information
	statefulsetGVK, err := apiutil.GVKForObject(&statefulset, r.Scheme)
	if err != nil {
		logger.Error(err, "Cannot get GVK for statefulset", "name", req.Name, "namespace", req.Namespace)
		return ctrl.Result{}, err
	}
	return commonReconcile(ctx, r.Client, statefulsetGVK, statefulset.ObjectMeta, requestCpuSum)
}

// SetupWithManager sets up the controller with the Manager.
func (r *AutoVPAStatefulSetReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&appsv1.StatefulSet{}, builder.WithPredicates(workloadPredicate)).
		Owns(&vpav1.VerticalPodAutoscaler{}, builder.WithPredicates(vpaPredicate)).
		Named("vpa_auto_statefulset").
		Complete(r)
}
